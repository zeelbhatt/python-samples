...
def _main():
    ...
    mask = np.zeros_like(rgb[:, :, :1])
    for i in range(600 - pixels_to_display, 600 + pixels_to_display):
        for j in range(960 - pixels_to_display, 960 + pixels_to_display):
            mask[i, j] = 1
    ...
    xyz_masked = np.multiply(xyz, mask)
